class Regex{
    ...
    findError(data){
        ...
        return unhandledRegex.test(data)?true:categoryIDRegex.test(data)?true:someThingWentWrong.test(data)?true:TechnicalGlitchregex.test(data)?true:internalServerError.test(data)?true:badGatewayError504.test(data)?true:serviceTemporyUnAvailable(data)?true:badGatewayError502(data)?true:false;
    }
